...
export const getNewBooks = async (page: number = 1): Promise<ApiResponse> => {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/newbook`, {
...
export const getPopularityBooks = async (
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/api/v1/popularity?page=${page}`,
    {
      next: { revalidate: 1800 },
    }
  );
...
export const getRecommendationBooks = async (): Promise<ApiResponse> => {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/api/v1/recommendation`,
    {
      next: { revalidate: 1800 },
    }
  );
